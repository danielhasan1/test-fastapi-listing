...
class EmployeeDao(GenericDao):
    ...
    def get_emp_ids_contain_full_name(self, full_name: str) -> list[int]:
        ...
        objs = (
            self._read_db.query(Employee.emp_no)
            .filter(
                func.concat(Employee.first_name, " ", Employee.last_name).contains(
                    full_name
                )
            )
            .all()
    ...
    def get_dept_by_manager(self, manager_emp_no) -> List[str]:
        objs = (
            self._read_db.query(DeptManager.dept_no)
            .filter(DeptManager.emp_no == manager_emp_no)
            .all()
        )
